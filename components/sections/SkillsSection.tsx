...
export const SkillsSection: React.FC = () => {
  ...
  return (
    <section id="skills" className="py-16 sm:py-20 lg:py-28 border-t border-white/[0.04]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 sm:gap-6 mb-5 sm:mb-6">
          {technicalSkills.map((cat, idx) => (
            <BentoCard key={idx} className="p-6 sm:p-7 md:p-8" hoverEffect={false}>
              <div className="mb-4 sm:mb-6">
                <span className="text-[10px] sm:text-[11px] font-medium tracking-[0.12em] text-[#5a7696] uppercase block mb-1.5">
                  CATEGORY
                </span>
                <h3 className="text-base sm:text-lg font-semibold text-[#EEEEEE] tracking-[-0.01em]">
                  {cat.title}
                </h3>
              </div>
              ...
              <div className="space-y-2 sm:space-y-2.5">
                {cat.skills.map((skill, sIdx) => (
                  <div
                    key={sIdx}
                    className="flex items-center gap-2.5 sm:gap-3 py-1.5 sm:py-2 border-b border-white/[0.04] last:border-0"
                  >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 sm:gap-6">
          ...
          <BentoCard className="lg:col-span-2 p-6 sm:p-8 lg:p-10" hoverEffect={false}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 sm:gap-x-8 gap-y-3 sm:gap-y-3.5">
              {softSkills.map((skill, idx) => (
                <div
                  key={idx}
                  className="flex items-center gap-2.5 sm:gap-3 py-1.5 sm:py-2 border-b border-white/[0.04]"
                >
                  <span className="w-1.5 h-1.5 rounded-full bg-[#36516D] flex-shrink-0" />
                  <span className="text-xs sm:text-[13px] font-medium text-[#EEEEEE] uppercase tracking-[0.08em]">
                    {skill}
          ...
          <BentoCard className="p-6 sm:p-8 lg:p-10 flex flex-col justify-between" hoverEffect={false}>
            <div>
              ...
              <div className="space-y-3.5 sm:space-y-4">
                {languages.map((lang, idx) => (
                  <div key={idx} className="flex items-center justify-between py-1.5 sm:py-2 border-b border-white/[0.04] last:border-0">
                    <span className="text-xs sm:text-sm font-semibold text-[#EEEEEE] uppercase tracking-[0.06em]">
                      {lang.name}
                    </span>
            ...
            <div className="pt-5 sm:pt-6 border-t border-white/[0.05] mt-5 sm:mt-6">
